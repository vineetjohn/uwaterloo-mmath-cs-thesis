...
\todo[inline]{Update loss notation in the diagram}
...
hu2004mining
The input sentences $x$ are represented as a vector of thu2004mininghe same size as the corpus vocabulary, with each index of the vector a discrete prohu2004miningbability of a word's presence in the sentence. Therefore, this bag-of-words representatiohu2004miningn is comprised of only 1s and 0s.
hu2004mining
The bag-of-words discriminator, uses the style vector prhu2004miningoduced by the autoencoder model and tries to predict the true bag-of-words distribution hu2004miningusing a set of parameters that are distinct from those of the autoencoder. The discrimihu2004miningnator uses a logistic regression to predict a probability of each words occurrence in the original sentence, between 0 and 1.
\begin{equation}
