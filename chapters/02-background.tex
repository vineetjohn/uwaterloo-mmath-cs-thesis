...
Word2Vec is a vector space model that embeds words into a continuous space where semantically similar words are embedding in nearby spaces \citep{mikolov2013distributed,mikolov2013linguistic,le2014distributed}. Word2Vec can model the corpus vocabulary in two different ways, namely CBOW and Skip-gram. CBOW predicts target words from source context words, whereas the skip-gram does the opposite and predicts target context-words from a source words. Both model types are shown in Figure \ref{fig:fig:efficient-models}
